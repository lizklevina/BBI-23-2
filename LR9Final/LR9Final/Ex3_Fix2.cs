...
class Country
{
    ...
    public virtual void PrintTopResponses(string question, string filePath)
    {
        Console.WriteLine($"\nТоп-5 наиболее часто встречающихся ответов на вопрос: {question}");

        if (!File.Exists(filePath))
        {
            Console.WriteLine("Файл с ответами не найден.");
            return;
        }
        ...
        try
        {
            ...
            responses = System.Text.Json.JsonSerializer.Deserialize<Dictionary<string, int>>(File.ReadAllText(filePath));
        }
        catch (Exception ex)
        {
...
class RadioPoll
{
    static void Main()
    {
        ...
        string[] filePaths = {
            Path.Combine(dataDirectory, "russia_animal.json"),
            Path.Combine(dataDirectory, "russia_character.json"),
            Path.Combine(dataDirectory, "russia_object.json"),
            Path.Combine(dataDirectory, "japan_animal.json"),
            Path.Combine(dataDirectory, "japan_character.json"),
            Path.Combine(dataDirectory, "japan_object.json")
        };
        ...
        russiaPoll.PrintTopResponses(questions[0], filePaths[0]);
        ...
        japanPoll.PrintTopResponses(questions[4], filePaths[4]);
        japanPoll.Responses.Clear(); // Очищаем словарь ответов


        japanPoll.FillResponses(questions[5]);
        SavePollData(japanPoll, Path.Combine(dataDirectory, "japan_object.json"));
        japanPoll.PrintTopResponses(questions[5], filePaths[5]);
        japanPoll.Responses.Clear(); // Очищаем словарь ответов
